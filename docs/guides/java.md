...
与其他编程风格指南一样，本文档涵盖的内容不仅限于格式化的美观问题，还包括各类惯例与编码规范。然而，本文档侧重于那些我们普遍遵守的**硬性规定**，而刻意回避那些无法通过人工或工具明确执行的"建议"。
...
2. 类的*成员*（member）一词泛指嵌套类、字段、方法或构造器，即类的所有顶层内容（初始化器除外）。
...
1. 其他所有空白字符在 `char`、字符串字面量以及文本块中均需转义。
2. **不能**使用制表符（Tab）进行缩进。
...
> **提示：** 不要因为担心某些程序可能无法正确处理非 ASCII 字符，就刻意降低代码的可读性。如果确实出现了问题，那是那些程序的**漏洞**，应该去**修复**它们。
...
**通配符导入**（即"按需导入"），无论是静态的还是非静态的，**一律不使用**。
...
如果同时存在静态导入和非静态导入，则两组之间用一个空行分隔，组内各条 `import` 语句之间没有空行。

每组内，导入的名称按 ASCII 码顺序排列。（**注意：** 这与 `import` *行*本身按 ASCII 码排序不同，因为 `.` 的 ASCII 码小于 `;`。）

#### 3.3.4 不对类使用静态导入

静态嵌套类不使用静态导入，而是用普通导入语句导入。

### 3.4 类声明

#### 3.4.1 恰好一个顶层类声明 {#s3.4.1-one-top-level-class}

每个顶层类独占一个源文件。

#### 3.4.2 类内容的排序

类的成员和初始化器的排列顺序对代码的可读性影响重大。然而，并不存在唯一正确的排列方式，不同的类可以采用不同的顺序。

重要的是，每个类都应遵循**某种合乎逻辑的顺序**，让维护者在被问到时能够解释清楚。例如，不应仅仅出于习惯就把新方法追加到类的末尾——那样只会产生"按添加时间排序"的结果，毫无逻辑性可言。

##### 3.4.2.1 重载：不拆散

同名方法必须紧挨着排列，中间不插入其他成员。多个构造器也适用同样的规则。即使方法或构造器之间的修饰符（如 `static` 或 `private`）有所不同，此规则依然适用。
...
2. `package` 声明和 `import` 语句（见第 3.2 节，[package 声明](#s3.2-package-declaration)，以及第 3.3 节，[import 语句](#s3.3-import-statements)）。
...
6. 双斜线（`//`）注释开头与其前面内容之间；双斜线注释开头与注释文本之间。两处均允许使用多个空格。
7. 声明中类型与标识符之间：`List<String> list`。
8. 数组初始化器两侧花括号内，*可选地*加空格：`new int[] {5, 6}` 和 `new int[] { 5, 6 }` 均有效。
9. 类型注解与 `[]` 或 `...` 之间。
...
> **提示：** 对齐有助于提高可读性，但为了维持对齐而刻意为之，会给将来埋下隐患。假设某次修改只涉及一行，如果该修改破坏了原有的对齐，就不应仅仅为了重新对齐而去改动附近无关的行。对无需修改的行引入格式化变更，会污染版本历史、拖慢代码审查速度，并加剧合并冲突。这些实际问题的优先级高于对齐本身。

### 4.7 分组括号：推荐使用 {#s4.7-grouping-parentheses}

只有当作者和审查者都认为省略括号不会引起误解、也不会影响可读性时，才可以省略可选的分组括号。不能假设每位读者都将整个 Java 运算符优先级表烂熟于心。
...
任何数组初始化器都*可以*选择写成"块状结构"的形式。例如，以下写法均有效（并非详尽列举）：
...
在旧式 switch 块中，每个语句组要么以明确的跳出语句结束（`break`、`continue`、`return` 或抛出异常），要么加上注释表明执行会（或可能会）继续到下一个语句组。任何能传达贯穿意图的注释均可（通常写 `// fall through`）。switch 块的最后一个语句组不需要加此注释。示例：
...
**例外：** 单个无参数的注解*可以*与签名的第一行写在同一行，例如：
...
文本块的内容可以超过[列宽限制](#s4.4-column-limit)。
...
对于"简单、显而易见"的成员和记录组件，例如 `getFoo()` 方法，*如果*确实没有什么值得说的，只是"返回 foo 的值"，则 Javadoc 是可选的。

> **重要：** 不能以此例外为由省略读者可能需要了解的重要信息。例如，对于名为 `canonicalName` 的记录组件，如果普通读者可能不知道"canonical name"（规范名称）是什么意思，就不应省略其文档（即便文档内容可能只是 `@param canonicalName the canonical name`）。
...
其他类、成员和记录组件可以根据需要或意愿编写 Javadoc。
